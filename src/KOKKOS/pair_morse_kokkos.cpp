...
compute_fpair(const F_FLOAT& rsq, const int& i, const int&j, const int& itype, const int& jtype) const {
  ...
  // U  =  d0 * [ Kokkos::Experimental::exp( -2*a*(x-r0)) - 2*exp(-a*(x-r0)) ]
  // f  = -2*a*d0*[ -Kokkos::Experimental::exp( -2*a*(x-r0) ) + exp( -a*(x-r0) ) ] * grad(r)
  //    = +2*a*d0*[  Kokkos::Experimental::exp( -2*a*(x-r0) ) - exp( -a*(x-r0) ) ] * grad(r)
